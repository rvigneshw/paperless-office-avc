...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 08, 2019 at 02:58 PM
...
INSERT INTO `papers` (`id`, `paper_type`, `department_id`, `subject`, `isApproved`, `content`, `associated_files_path`, `priority`, `created_at`, `paper_submitted_person`, `amount`, `status_of_manager`, `status_of_director`, `status_of_principal`, `status_of_secretary`, `last_modified_person`, `updated_at`, `returned_for_query`, `resubmission_count`) VALUES
(4, 1, 11, 'kjbwjfjodbfdbf', 0, '&lt;p&gt;dlkfodbfdbf&lt;/p&gt;&lt;p&gt;&lt;!--EndFragment--&gt;&lt;br&gt;&lt;/p&gt;', 'uploads/kjbwjfjodbfdbf-1575203478/', 3, '2019-12-01 18:01:18', 'HOD-CSE', 8789, 2, 1, 0, 0, 'manager', '2019-12-08 11:55:34', 0, 1);
...
(8, 4, '<b>manager Asked:</b>ohdifsa', '<b>manager Answered:</b>kgvhbjnk', '2019-12-07 23:32:22', '2019-12-07 23:33:04'),
(9, 4, '<b>manager Asked:</b>rszdxgcgbhkj', '<b>manager Answered:</b>jgvsixs', '2019-12-08 11:53:51', '2019-12-08 11:55:34');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `paper_all`  AS  select `papers`.`id` AS `id`,`papertype`.`paper_type_name` AS `paper_type_name`,`department`.`department_name` AS `department_name`,`papers`.`subject` AS `subject`,`papers`.`isApproved` AS `isApproved`,`papers`.`content` AS `content`,`papers`.`associated_files_path` AS `associated_files_path`,`papers`.`priority` AS `priority`,`papers`.`created_at` AS `created_at`,`papers`.`paper_submitted_person` AS `paper_submitted_person`,`papers`.`amount` AS `amount`,`papers`.`status_of_manager` AS `status_of_manager`,`papers`.`resubmission_count` AS `resubmission_count`,`papers`.`status_of_director` AS `status_of_director`,`papers`.`status_of_principal` AS `status_of_principal`,`papers`.`status_of_secretary` AS `status_of_secretary`,`papers`.`last_modified_person` AS `last_modified_person`,`papers`.`updated_at` AS `updated_at` from ((`papers` join `papertype`) join `department`) where ((`papers`.`paper_type` = `papertype`.`paper_type_id`) and (`papers`.`department_id` = `department`.`dept_id`)) ;
